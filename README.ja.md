...
> ⚠️ **Claude Code 専用（v1）。** `statusLine` および `hooks` は Claude Code 固有の機構です。
> Cursor、Codex CLI、OpenCode、Gemini CLI 等の他エージェントでは動作しません。
> Codex CLI アダプターは **experimental** として同梱されています（下記参照）。
...
## Claude Code 専用（v1）について

`statusLine` および `hooks.UserPromptSubmit` / `hooks.PostToolUse` は Claude Code 固有の機構です。以下では動作しません：

- Codex CLI（hooks はあるが、コンテキストシグナルが異なる — 下記 experimental 参照）

...
| 対象 | 動作要件 | サポート |
|------|----------|----------|
| **Claude Code** | `statusLine` と `hooks` をサポートする最近のバージョン、`bash`、`jq` | **stable** |
| **Codex CLI / Codex Desktop** | hooks と rollout JSONL transcript が使える Codex、`bash`、`jq`、`awk` | **experimental** |

Codex 対応は実際に使えますが、まだ stable surface ではありません。
`adapters/codex/install.sh` または `bin/selfctx install-codex` で入れ、Codex が
hook trust を求めたら承認してください。
...
- インストール済みの `codex` CLI で config が parse できること
